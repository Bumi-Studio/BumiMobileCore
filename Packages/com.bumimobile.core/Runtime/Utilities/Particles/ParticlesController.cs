...
namespace BumiMobile
{
    public class ParticlesController : MonoBehaviour
    {
        ...
        private static ParticleCase ActivateParticle(Particle particle, float delay = 0)
        {
            ...
            GameObject particleObject = particle.GetInstance();
            ...
            {
                Debug.LogError($"[Particles System]: Particle '{particle.ParticleName}' does not contain a ParticleSystem component.");
                particle.ReleaseInstance(particleObject);
                return null;
            }

            ParticleCase particleCase = ActivateParticle(particleSystem, delay, true);
            if (particleCase != null)
            {
                particleCase.Disabled += () => particle.ReleaseInstance(particleObject);
            }
