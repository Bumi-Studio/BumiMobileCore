...
namespace BumiMobile
{
    ...
    {
        public static readonly string ProjectFolderPath = Application.dataPath.Replace("/Assets", "/");
        ...
        public static GenericMenu GetSubTypeMenu(Type parentType, Action<Type> selectAction, Type selectedType = null, bool showAbstract = false)
        {
            ...
            if (!parentType.IsAbstract)
                menu.AddItem(new GUIContent(parentType.ToString()), parentType == selectedType, () => selectAction(parentType));
            ...
            {
                SubType(ref menu, itemTypes, baseType, selectAction, string.Empty, selectedType, showAbstract);
            }
        ...
        private static void SubType(ref GenericMenu menu, Type[] itemTypes, Type baseType, Action<Type> selectAction, string defaultPath = "", Type selectedType = null, bool showAbstract = false)
        {
            ...
            if (subItemTypes.Length > 0)
            {
                if (showAbstract || !baseType.IsAbstract)
                    menu.AddItem(new GUIContent(defaultPath + baseType + "/" + baseType), baseType == selectedType, () => selectAction(baseType));

                foreach (Type subType in subItemTypes)
                {
                    SubType(ref menu, itemTypes, subType, selectAction, defaultPath + baseType + "/", selectedType);
                }
            }
            else
            {
                if (showAbstract || !baseType.IsAbstract)
                    menu.AddItem(new GUIContent(defaultPath + baseType), baseType == selectedType, () => selectAction(baseType));
            }
        }
        ...
        {
            string[] assets = AssetDatabase.FindAssets((string.IsNullOrEmpty(name) ? string.Empty : name + " ") + "t:" + typeof(T).Name);
            if (assets.Length > 0)
            {
        ...
        {
            string[] assetsPath = AssetDatabase.FindAssets((string.IsNullOrEmpty(name) ? string.Empty : name + " ") + "t:" + typeof(T).Name);
        ...
        public static T CreateAsset<T>(Type type, string path, bool refresh = false) where T : ScriptableObject
        {
        ...
        private static string FindSubfolder(string folderName, string rootPath)
        {
            ...
            {
                if (string.Compare(Path.GetFileName(subdirectory), folderName, StringComparison.Ordinal) == 0)
                    return subdirectory;

                string result = FindSubfolder(folderName, subdirectory);
